...
let $possibleCloserDateline := 
  ...
  for $possibleCloser in $doc/tei:p[last()][matches(.,'(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}')]
  let $possibleCloserDate := (tokenize(functx:trim(serialize(functx:get-matches(normalize-space(serialize(data($possibleCloser))),'(January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2},\s+\d{4}'))),'\s\s+'))[1]
  

  let $pcdYear := substring-after($possibleCloserDate, ', ')
  

  let $pcdMonth :=
          switch (substring-before($possibleCloserDate,' '))
           case "January" return "01"
           case "February" return "02"
           case "March" return "03"
           case "April" return "04"
    ...
    let $pcdDay := data(substring-after(substring-before($possibleCloserDate, ','),' '))
  ...
  return 
  <closer>
    <dateline><date when="{$pcdISO}">{$possibleCloserDate}</date>.</dateline>
  </closer>
  ...
  ('  - [x] Edit existing `p` to `dateline` (Move to `closer` as necessary):&#10;```xml&#10;',$unmarkedDateline,'&#10;```'),'&#10;'))
        ...
        then concat('- [x] ',$url,'&#10;',string-join(
  ('  - [x] Edit existing `p` to `closer`:&#10;```xml',serialize($possibleCloserDateline),'```**OR**&#10;  - [x] Add `dateline`:&#10;```xml&#10;<dateline>',$newDateline,'</dateline>&#10;```'),'&#10;'))
        else
