...
: <date> from <head> (appending years from chapter headings, as necessary) or 2) searching the head for the 
...
let $docIssues :=
  ...
  let $chapterParent := $doc/parent::tei:div
  let $chapterYear := 
  tokenize(functx:trim(serialize(functx:get-matches(normalize-space(serialize(data($chapterParent/tei:head))),'\d{4}'))))[1]

  let $dResult :=
  
    let $d := 
    
    if (matches(normalize-space(serialize(data($head))),'(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}, \d{4}'))
    then
    (tokenize(functx:trim(serialize(functx:get-matches(normalize-space(serialize(data($head))),'(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}, \d{4}'))),'\s\s+'))[1]
    else
      if (matches(normalize-space(serialize(data($head))),'(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}'))
      then
      concat((tokenize(functx:trim(serialize(functx:get-matches(normalize-space(serialize(data($head))),'(January|February|March|April|May|June|July|August|September|October|November|December) \d{1,2}'))),'\s\s+'))[1],', ',$chapterYear)
     else null
